...
substr_dat <- read_csv("data/substrate_data.csv") %>% 
  ...
  summarise(mean_substr = mean(substrate_code), 
            sd_substr = sd(substrate_code)) %>% 
...
dat <- read_csv("data/sea_cucumber_data.csv", na = c("", "NA", "ND")) %>% 
  group_by(latitude, longitude) %>% 
  summarise(sc_density = mean(sc_density, na.rm = TRUE)) %>% 
  na.omit %>% 
  # create y/z for the hurdle model and fix longitude (given as E)
  mutate(longitude = -longitude,
...
depth_sf <- st_difference(depth_sf, st_combine(map_sf)) %>% st_cast("POLYGON")
...
res(ras) <- 10
depth_ras <- fasterize::fasterize(depth_sf, ras)
...
```{r abund_plot, fig.width = 6.3, fig.height = 9.5}
ggplot() + 
...
```{r habitat_plot, fig.width = 6.3, fig.height = 9.5}
ggplot() + 
  geom_sf(data = map_sf, colour = "lightgrey", fill = "lightgrey") +
