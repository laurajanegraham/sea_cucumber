...
We need a spatial mesh to create the spatial random field. This will be restricted to the convex hull around the prediction points, and such that land forms a barrier. The range value is $\rho =$ `r range0` and the sd value is `\sigma = 3`. Both have low probability (0.05). 
...
# based on render parameters - either runs the PC prior version with a specified range, or runs a standard uninformative prior. 
...
f_timest <- alldata ~ -1 + z_intercept + y_intercept + 
  f(date, model = "rw2") + 
  f(transect, model = "iid") + 
  f(site, model = "iid")

m_timest <- inla(f_timest, family = c("gamma", "binomial"),
                data = inla.stack.data(stack_yz),
                control.predictor = list(A = inla.stack.A(stack_yz)),
                control.compute = list(dic = TRUE, waic = TRUE, config = TRUE))
...
f_sptempst <- alldata ~ -1 + z_intercept + y_intercept + 
  f(date, model = "rw2") +
  f(site, model = "iid") +
  f(transect, model = "iid") + 
  f(z_field, model = spde) +
...
m_sptempst <- inla(f_sptempst, family = c("gamma", "binomial"),
                  data = inla.stack.data(stack_yz),
                  control.predictor = list(A = inla.stack.A(stack_yz)),
                  control.compute = list(dic = TRUE, waic = TRUE, config = TRUE))

mods <- list(null = m_null, nulls = m_nulls, 
             time = m_time, times = m_times, timest = m_timest,
             spatial = m_spatial, spatials = m_spatials, 
             sptemp = m_sptemp, sptemps = m_sptemps, 
             sptempst = m_sptempst)

fname = ifelse(pcprior, paste0("results/pc",range0,"_out.Rda", "results/uninf_out.Rda"))
